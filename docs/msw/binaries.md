...
* [MSYS2](https://www.msys2.org/) MinGW 12.2.0, 13.2.0 and 14.2.0.

Getting the files
-----------------

First, you need to get the correct files. You will always need the
`wxWidgets-3.2.7-headers.7z` one but the rest depends on your compiler version
...
`wxMSW-3.2.7_vc14x_Dev.7z` and `wxMSW-3.2.7_vc14x_ReleaseDLL.7z`.

All binaries are available at:

https://www.wxwidgets.org/downloads#v3.2.7_msw

Once you have the files you need, unzip all of them into the same directory, for
example `c:\wx\3.2.7`. You should have `include` and `lib` subdirectories under
