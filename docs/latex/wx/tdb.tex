...
recommended classes for doing database/ODBC work with wxWidgets. These new 
classes replace the wxWidgets v1.6x classes wxDatabase. Documentation for the 
...
\subsection{Different ODBC Class Libraries in wxWidgets}
...
wxWidgets v2.x, they have become the recommended wxWidgets classes for database 
access.

An older version of some classes ported over from wxWidgets v1.68 still exist 
...
\normalboxd{The more sophisticated wxODBC classes (wxDb/wxDbTable) are the 
recommended classes for doing database/ODBC work with wxWidgets. These new 
classes replace the wxWidgets v1.6x classes wxDatabase.
...
wxWidgets provides a set of classes for accessing a subset of Microsoft's ODBC (Open Database Connectivity)
...
classes may be provided in wxWidgets at a later date.
...
the wxWidgets ODBC wrapper API. Even though the databases may not be
...
A sample is distributed with wxWidgets in {\tt samples/odbc}. You will need to install
...
types, not currently supported in wxWidgets.
