...
\item wxDF\_TEXT or wxDF\_OEMTEXT: returns a pointer to new memory containing a null-terminated text string.
\item wxDF\_BITMAP: returns a new wxBitmap.
\end{itemize}
...
\begin{itemize}\itemsep=0pt
\item wxDF\_TEXT or wxDF\_OEMTEXT: {\it data} is a null-terminated text string.
\item wxDF\_BITMAP: {\it data} is a wxBitmap.
\item wxDF\_DIB: {\it data} is a wxBitmap. The bitmap is converted to a DIB (device independent bitmap).
\item wxDF\_METAFILE: {\it data} is a wxMetafile. {\it width} and {\it height} are used to give recommended dimensions.
\end{itemize}
