...
Note that wxWidgets currently uses the [NanoSVG][] library (or, optionally,
[LunaSVG][], which requires C++17) for SVG support and so doesn't support all SVG
standard features and you may need to simplify or tweak the SVG
files to make them appear correctly.
