...
/**
    ...
    determined as follows:
        - For the Latin-1 letters they correspond to the @e upper case value of
        the letter.
        - For the other keys producing printable characters, the key code
        depends on whether the key produces an ASCII character or not: if it
        does, this character is used as its code, so that a key marked with
        e.g. `$` in French keyboard layout generates this key code. But if it
        does not, e.g. when the same physical key is pressed in Ukrainian
        keyboard layout where it would normally produce the non-ASCII letter
        `ï`, then the key code is the code of the corresponding character in
        the standard US layout, i.e. `]`. This also applies to the letters,
        i.e. the key code producing the Cyrillic letter `ц` in Ukrainian layout
        generates the key code `W` corresponding to the letter of this key in
        the US layout.
