...
/**
    ...
    Event sent when the resolution or DPI (dots per inch) of the monitor a window is on changes.

    When the resolution of the monitor changes, the event is sent to the wxTopLevelWindow.
    When the DPI changes, it is send to the wxTopLevelWindow and all its children.

    DPI related events are generated by wxMSW port only and only if the
    MSW application is marked as being "per-monitor DPI aware", i.e. contains a
    @c dpiAwareness tag with the value "PerMonitorV2" in its manifest (see
    Microsoft <a href="https://msdn.microsoft.com/en-us/library/windows/desktop/aa374191.aspx">"Application Manifests" documentation</a>
    for more details).
