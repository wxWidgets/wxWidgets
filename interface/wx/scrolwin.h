...
/**
    ...
    @beginEventEmissionTable{wxScrollWinPanEvent}
    @event{EVT_SCROLLWIN_PAN(func)}
        Process @c wxEVT_SCROLLWIN_PAN pan events after finger scrolling on touch screens.
    @endEventTable
    ...
    @note
        Since version 3.3.1 wxScrolled<> also intercepts wxPanGestureEvents
        and translates these into wxScrollWinPanEvents to allow scrolling
        following a user pan gesture (scrolling with finger on a touch screen).
    ...
    @see wxScrollBar, wxClientDC, wxPaintDC,
         ...
         wxPanGestureEvent, wxScrollWinEvent, wxScrollWinPanEvent
*/
template<class T>
