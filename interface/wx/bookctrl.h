...
public:
    ...
    */
    ...
    /**
        ...
        The window will be hidden, though, so you may need to call Show()
        after e.g. reparenting it.
...
/**
    ...
    The page `CHANGING` events are sent before the current page is changed.
    ...
    of the old page are invalid. However the handler for this event does not
    have access to the new page being selected as this information is not
    always provided by the underlying native control.

    The page `CHANGED` events are sent after the page has been changed and the
    program cannot veto it any more, it just informs it about the page change.
    However the handler for this event has access to the newly selected page.

    To summarize, if the program is interested in validating the page values
    before allowing the user to change it, it should process the page changing
    event. If it just needs to react to the page change after it happened,
    handling page changed events is enough. It is also perfectly possible to
    handle both of these events, for different purposes.
