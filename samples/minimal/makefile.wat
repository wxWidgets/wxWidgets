...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\minimal.exe

$(OBJS)\minimal.exe :  $(MINIMAL_OBJECTS)
	...
	@%append $(OBJS)\minimal.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(MINIMAL_OBJECTS)) do @%append $(OBJS)\minimal.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\minimal.lbc library %i
	@%append $(OBJS)\minimal.lbc
	@for %i in () do @%append $(OBJS)\minimal.lbc option stack=%i
	wlink @$(OBJS)\minimal.lbc

$(OBJS)\minimal_minimal.obj :  .AUTODEPEND .\minimal.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(MINIMAL_CXXFLAGS) $<
