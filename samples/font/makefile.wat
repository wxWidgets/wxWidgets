...
$(OBJS)\font.exe :  $(FONT_OBJECTS)
	@%create $(OBJS)\font.lbc
	@%append $(OBJS)\font.lbc option quiet
	@%append $(OBJS)\font.lbc name $^@
	@%append $(OBJS)\font.lbc option caseexact
	@%append $(OBJS)\font.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(FONT_OBJECTS)) do @%append $(OBJS)\font.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\font.lbc library %i
	@%append $(OBJS)\font.lbc
	@for %i in () do @%append $(OBJS)\font.lbc option stack=%i
	wlink @$(OBJS)\font.lbc

$(OBJS)\font_font.obj :  .AUTODEPEND .\font.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(FONT_CXXFLAGS) $<
