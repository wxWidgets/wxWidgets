...
$(OBJS)\dnd.exe :  $(DND_OBJECTS)
	@%create $(OBJS)\dnd.lbc
	@%append $(OBJS)\dnd.lbc option quiet
	@%append $(OBJS)\dnd.lbc name $^@
	@%append $(OBJS)\dnd.lbc option caseexact
	@%append $(OBJS)\dnd.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(DND_OBJECTS)) do @%append $(OBJS)\dnd.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\dnd.lbc library %i
	@%append $(OBJS)\dnd.lbc
