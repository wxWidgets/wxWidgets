...
$(OBJS)\taskbar.exe :  $(TASKBAR_OBJECTS)
	...
	@%append $(OBJS)\taskbar.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(TASKBAR_OBJECTS)) do @%append $(OBJS)\taskbar.lbc file %i
	@for %i in ( $(__WXLIB_ADV_p)  $(__WXLIB_HTML_p)  $(__WXLIB_XML_p)  $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\taskbar.lbc library %i
	@%append $(OBJS)\taskbar.lbc
	@for %i in () do @%append $(OBJS)\taskbar.lbc option stack=%i
	wlink @$(OBJS)\taskbar.lbc

$(OBJS)\taskbar_tbtest.obj :  .AUTODEPEND .\tbtest.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(TASKBAR_CXXFLAGS) $<
