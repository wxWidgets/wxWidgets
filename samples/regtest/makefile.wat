...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\regtest.exe

$(OBJS)\regtest.exe :  $(REGTEST_OBJECTS)
	...
	@%append $(OBJS)\regtest.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(REGTEST_OBJECTS)) do @%append $(OBJS)\regtest.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\regtest.lbc library %i
	@%append $(OBJS)\regtest.lbc
	@for %i in () do @%append $(OBJS)\regtest.lbc option stack=%i
	wlink @$(OBJS)\regtest.lbc

$(OBJS)\regtest_regtest.obj :  .AUTODEPEND .\regtest.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(REGTEST_CXXFLAGS) $<
