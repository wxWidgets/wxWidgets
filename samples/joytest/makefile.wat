...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\joytest.exe

$(OBJS)\joytest.exe :  $(JOYTEST_OBJECTS)
	...
	@%append $(OBJS)\joytest.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(JOYTEST_OBJECTS)) do @%append $(OBJS)\joytest.lbc file %i
	@for %i in ( $(__WXLIB_ADV_p)  $(__WXLIB_HTML_p)  $(__WXLIB_XML_p)  $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\joytest.lbc library %i
	@%append $(OBJS)\joytest.lbc
	@for %i in () do @%append $(OBJS)\joytest.lbc option stack=%i
	wlink @$(OBJS)\joytest.lbc

$(OBJS)\joytest_joytest.obj :  .AUTODEPEND .\joytest.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(JOYTEST_CXXFLAGS) $<
