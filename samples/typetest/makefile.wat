...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\typetest.exe

$(OBJS)\typetest.exe :  $(TYPETEST_OBJECTS)
	...
	@%append $(OBJS)\typetest.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(TYPETEST_OBJECTS)) do @%append $(OBJS)\typetest.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\typetest.lbc library %i
	@%append $(OBJS)\typetest.lbc
	@for %i in () do @%append $(OBJS)\typetest.lbc option stack=%i
	wlink @$(OBJS)\typetest.lbc

$(OBJS)\typetest_typetest.obj :  .AUTODEPEND .\typetest.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(TYPETEST_CXXFLAGS) $<
