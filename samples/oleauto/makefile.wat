...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\oleauto.exe

$(OBJS)\oleauto.exe :  $(OLEAUTO_OBJECTS)
	...
	@%append $(OBJS)\oleauto.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(OLEAUTO_OBJECTS)) do @%append $(OBJS)\oleauto.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\oleauto.lbc library %i
	@%append $(OBJS)\oleauto.lbc
	@for %i in () do @%append $(OBJS)\oleauto.lbc option stack=%i
	wlink @$(OBJS)\oleauto.lbc

$(OBJS)\oleauto_oleauto.obj :  .AUTODEPEND .\oleauto.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(OLEAUTO_CXXFLAGS) $<
