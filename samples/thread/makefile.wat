...
$(OBJS)\thread.exe :  $(THREAD_OBJECTS)
	@%create $(OBJS)\thread.lbc
	@%append $(OBJS)\thread.lbc option quiet
	@%append $(OBJS)\thread.lbc name $^@
	@%append $(OBJS)\thread.lbc option caseexact
	@%append $(OBJS)\thread.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(THREAD_OBJECTS)) do @%append $(OBJS)\thread.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\thread.lbc library %i
	@%append $(OBJS)\thread.lbc
	@for %i in () do @%append $(OBJS)\thread.lbc option stack=%i
	wlink @$(OBJS)\thread.lbc

$(OBJS)\thread_thread.obj :  .AUTODEPEND .\thread.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(THREAD_CXXFLAGS) $<
