...
$(OBJS)\htlbox.exe :  $(HTLBOX_OBJECTS)
	...
	@%append $(OBJS)\htlbox.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(HTLBOX_OBJECTS)) do @%append $(OBJS)\htlbox.lbc file %i
	@for %i in ( $(__WXLIB_HTML_p)  $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\htlbox.lbc library %i
	@%append $(OBJS)\htlbox.lbc
	@for %i in () do @%append $(OBJS)\htlbox.lbc option stack=%i
	wlink @$(OBJS)\htlbox.lbc

$(OBJS)\htlbox_htlbox.obj :  .AUTODEPEND .\htlbox.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(HTLBOX_CXXFLAGS) $<
