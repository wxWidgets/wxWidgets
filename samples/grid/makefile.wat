...
$(OBJS)\grid.exe :  $(GRID_OBJECTS)
	@%create $(OBJS)\grid.lbc
	@%append $(OBJS)\grid.lbc option quiet
	@%append $(OBJS)\grid.lbc name $^@
	@%append $(OBJS)\grid.lbc option caseexact
	@%append $(OBJS)\grid.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(GRID_OBJECTS)) do @%append $(OBJS)\grid.lbc file %i
	@for %i in ( $(__WXLIB_ADV_p)  $(__WXLIB_HTML_p)  $(__WXLIB_XML_p)  $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\grid.lbc library %i
	@%append $(OBJS)\grid.lbc
	@for %i in () do @%append $(OBJS)\grid.lbc option stack=%i
	wlink @$(OBJS)\grid.lbc

$(OBJS)\grid_griddemo.obj :  .AUTODEPEND .\griddemo.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(GRID_CXXFLAGS) $<
