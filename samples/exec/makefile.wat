...
$(OBJS)\exec.exe :  $(EXEC_OBJECTS)
	@%create $(OBJS)\exec.lbc
	@%append $(OBJS)\exec.lbc option quiet
	@%append $(OBJS)\exec.lbc name $^@
	@%append $(OBJS)\exec.lbc option caseexact
	@%append $(OBJS)\exec.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(EXEC_OBJECTS)) do @%append $(OBJS)\exec.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\exec.lbc library %i
	@%append $(OBJS)\exec.lbc
	@for %i in () do @%append $(OBJS)\exec.lbc option stack=%i
	wlink @$(OBJS)\exec.lbc

$(OBJS)\exec_exec.obj :  .AUTODEPEND .\exec.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(EXEC_CXXFLAGS) $<
