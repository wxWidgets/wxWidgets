...
$(OBJS)\image.exe :  $(IMAGE_OBJECTS)
	@%create $(OBJS)\image.lbc
	@%append $(OBJS)\image.lbc option quiet
	@%append $(OBJS)\image.lbc name $^@
	@%append $(OBJS)\image.lbc option caseexact
	@%append $(OBJS)\image.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(IMAGE_OBJECTS)) do @%append $(OBJS)\image.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\image.lbc library %i
	@%append $(OBJS)\image.lbc
...
$(OBJS)\image_image.obj :  .AUTODEPEND .\image.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(IMAGE_CXXFLAGS) $<
