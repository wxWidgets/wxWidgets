...
$(OBJS)\toolbar.exe :  $(TOOLBAR_OBJECTS)
	...
	@%append $(OBJS)\toolbar.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(TOOLBAR_OBJECTS)) do @%append $(OBJS)\toolbar.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\toolbar.lbc library %i
	@%append $(OBJS)\toolbar.lbc
	@for %i in () do @%append $(OBJS)\toolbar.lbc option stack=%i
	wlink @$(OBJS)\toolbar.lbc

$(OBJS)\toolbar_toolbar.obj :  .AUTODEPEND .\toolbar.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(TOOLBAR_CXXFLAGS) $<
