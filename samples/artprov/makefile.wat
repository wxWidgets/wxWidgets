...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\arttest.exe

$(OBJS)\arttest.exe :  $(ARTTEST_OBJECTS)
	...
	@%append $(OBJS)\arttest.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(ARTTEST_OBJECTS)) do @%append $(OBJS)\arttest.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\arttest.lbc library %i
	@%append $(OBJS)\arttest.lbc
	@for %i in () do @%append $(OBJS)\arttest.lbc option stack=%i
	wlink @$(OBJS)\arttest.lbc

$(OBJS)\arttest_arttest.obj :  .AUTODEPEND .\arttest.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(ARTTEST_CXXFLAGS) $<
