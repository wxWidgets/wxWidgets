...
$(OBJS)\combo.exe :  $(COMBO_OBJECTS)
	@%create $(OBJS)\combo.lbc
	@%append $(OBJS)\combo.lbc option quiet
	@%append $(OBJS)\combo.lbc name $^@
	@%append $(OBJS)\combo.lbc option caseexact
	@%append $(OBJS)\combo.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(COMBO_OBJECTS)) do @%append $(OBJS)\combo.lbc file %i
	@for %i in ( $(__WXLIB_ADV_p)  $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\combo.lbc library %i
	@%append $(OBJS)\combo.lbc
...
$(OBJS)\combo_combo.obj :  .AUTODEPEND .\combo.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(COMBO_CXXFLAGS) $<
