...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\statbar.exe

$(OBJS)\statbar.exe :  $(STATBAR_OBJECTS)
	...
	@%append $(OBJS)\statbar.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(STATBAR_OBJECTS)) do @%append $(OBJS)\statbar.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\statbar.lbc library %i
	@%append $(OBJS)\statbar.lbc
	@for %i in () do @%append $(OBJS)\statbar.lbc option stack=%i
	wlink @$(OBJS)\statbar.lbc

$(OBJS)\statbar_statbar.obj :  .AUTODEPEND .\statbar.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(STATBAR_CXXFLAGS) $<
