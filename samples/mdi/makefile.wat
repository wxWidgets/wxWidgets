...
$(OBJS)\mdi.exe :  $(MDI_OBJECTS)
	@%create $(OBJS)\mdi.lbc
	@%append $(OBJS)\mdi.lbc option quiet
	@%append $(OBJS)\mdi.lbc name $^@
	@%append $(OBJS)\mdi.lbc option caseexact
	@%append $(OBJS)\mdi.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(MDI_OBJECTS)) do @%append $(OBJS)\mdi.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\mdi.lbc library %i
	@%append $(OBJS)\mdi.lbc
	@for %i in () do @%append $(OBJS)\mdi.lbc option stack=%i
	wlink @$(OBJS)\mdi.lbc

$(OBJS)\mdi_mdi.obj :  .AUTODEPEND .\mdi.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(MDI_CXXFLAGS) $<
