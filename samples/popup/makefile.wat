...
$(OBJS)\popup.exe :  $(POPUP_OBJECTS)
	@%create $(OBJS)\popup.lbc
	@%append $(OBJS)\popup.lbc option quiet
	@%append $(OBJS)\popup.lbc name $^@
	@%append $(OBJS)\popup.lbc option caseexact
	@%append $(OBJS)\popup.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(POPUP_OBJECTS)) do @%append $(OBJS)\popup.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\popup.lbc library %i
	@%append $(OBJS)\popup.lbc
	@for %i in () do @%append $(OBJS)\popup.lbc option stack=%i
	wlink @$(OBJS)\popup.lbc

$(OBJS)\popup_popup.obj :  .AUTODEPEND .\popup.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(POPUP_CXXFLAGS) $<
