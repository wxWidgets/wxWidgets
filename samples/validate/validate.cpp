...
// Helper that converts a space delimited string to wxArrayString.
static inline wxArrayString ToArrayString(const wxString& str)
{
    wxArrayString array;
...
void TextValidatorDialog::OnKillFocus(wxFocusEvent &event)
{
    ...
    if ( txtCtrl && txtCtrl->IsModified() )
    {
        ...
        {
            m_includes = ToArrayString(txtCtrl->GetValue());
        }
        else if ( id == Id_ExcludeListTxt )
        {
            m_excludes = ToArrayString(txtCtrl->GetValue());
        }
    }
