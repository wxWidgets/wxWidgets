...
$(OBJS)\text.exe :  $(TEXT_OBJECTS)
	@%create $(OBJS)\text.lbc
	@%append $(OBJS)\text.lbc option quiet
	@%append $(OBJS)\text.lbc name $^@
	@%append $(OBJS)\text.lbc option caseexact
	@%append $(OBJS)\text.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(TEXT_OBJECTS)) do @%append $(OBJS)\text.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\text.lbc library %i
	@%append $(OBJS)\text.lbc
	@for %i in () do @%append $(OBJS)\text.lbc option stack=%i
	wlink @$(OBJS)\text.lbc

$(OBJS)\text_text.obj :  .AUTODEPEND .\text.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(TEXT_CXXFLAGS) $<
