...
$(OBJS)\vstest.exe :  $(VSTEST_OBJECTS)
	...
	@%append $(OBJS)\vstest.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(VSTEST_OBJECTS)) do @%append $(OBJS)\vstest.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\vstest.lbc library %i
	@%append $(OBJS)\vstest.lbc
	@for %i in () do @%append $(OBJS)\vstest.lbc option stack=%i
	wlink @$(OBJS)\vstest.lbc

$(OBJS)\vstest_vstest.obj :  .AUTODEPEND .\vstest.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(VSTEST_CXXFLAGS) $<
