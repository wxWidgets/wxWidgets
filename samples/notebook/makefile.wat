...
$(OBJS)\notebook.exe :  $(NOTEBOOK_OBJECTS)
	@%create $(OBJS)\notebook.lbc
	@%append $(OBJS)\notebook.lbc option quiet
	@%append $(OBJS)\notebook.lbc name $^@
	@%append $(OBJS)\notebook.lbc option caseexact
	@%append $(OBJS)\notebook.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(NOTEBOOK_OBJECTS)) do @%append $(OBJS)\notebook.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\notebook.lbc library %i
	@%append $(OBJS)\notebook.lbc
	@for %i in () do @%append $(OBJS)\notebook.lbc option stack=%i
	wlink @$(OBJS)\notebook.lbc

$(OBJS)\notebook_notebook.obj :  .AUTODEPEND .\notebook.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(NOTEBOOK_CXXFLAGS) $<
