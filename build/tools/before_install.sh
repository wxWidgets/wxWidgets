...
case $(uname -s) in
    Linux)
        ...
        if [ -f /etc/redhat-release ]; then
            dnf install -y ${WX_EXTRA_PACKAGES} gawk expat-devel findutils g++ git-core gspell-devel gstreamer1-plugins-base-devel gtk3-devel make libayatana-appindicator-gtk3-devel libcurl-devel libjpeg-devel libnotify-devel libpng-devel libSM-devel libsecret-devel libtiff-devel SDL-devel wayland-protocols-devel webkit2gtk4.1-devel weston xwayland-run zlib-devel
        fi
        ;;
