...
wxObject *wxNotebookXmlHandler::DoCreateResource()
{
    ...
    else
    {
        ...
        wxNotebook* const nb =
            m_instance ? wxDynamicCast(m_instance, wxNotebook)
                       : new wxNotebook;
        if ( !nb )
        {
            if ( factoryOwned )
            ...
            ReportError("provided instance is not a wxNotebook");
            return nullptr;
        }
        ...
        // SetupWindow() invokes virtual window APIs and may synchronously
        // destroy either the new book or its parent.
        SetupWindow(nb);
