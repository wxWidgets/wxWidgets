...
// globals variables needed for x11 raw key codes to latin chars transformations
#ifdef wxHAS_XKB
xkb_context *g_ctx = nullptr;
xkb_keymap *g_keymap = nullptr;
xkb_state *g_state = nullptr;
#endif
                           ...
                           GdkEventKey *gdk_event)
{
    ...
    if (gdk_keyval_to_unicode(gdk_event->keyval))
    {
        ...
        if (!g_state)
        {
            ...
            names.layout = "us";
            g_keymap = xkb_keymap_new_from_names(g_ctx, &names, XKB_KEYMAP_COMPILE_NO_FLAGS);
...
bool wxXKBModule::OnInit()
{
    return true;
}

void wxXKBModule::OnExit()
{
#ifdef wxHAS_XKB
    if (g_state) { xkb_state_unref(g_state); }
    if (g_keymap) { xkb_keymap_unref(g_keymap); }
    if (g_ctx) { xkb_context_unref(g_ctx); }
#endif
}
