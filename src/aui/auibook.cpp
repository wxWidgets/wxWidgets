...
void wxAuiTabCtrl::OnLeftDown(wxMouseEvent& evt)
{
    ...
    if (buttonUnderMouse && buttonUnderMouse != pressedButton)
    {
        buttonUnderMouse->curState |= wxAUI_BUTTON_STATE_PRESSED;
        Refresh();
        Update();
    }
    else if (pressedButton)
    {
        pressedButton->curState &= ~wxAUI_BUTTON_STATE_PRESSED;
        Refresh();
        Update();
    }
}
...
void wxAuiTabCtrl::OnMotion(wxMouseEvent& evt)
{
    ...
    if (button && !(button->curState & wxAUI_BUTTON_STATE_DISABLED))
    {
        ...
        {
            hoverButton->curState &= ~wxAUI_BUTTON_STATE_HOVER;
            Refresh();
            Update();
        }
...
void wxAuiTabCtrl::ResetHoverButton()
{
    auto* const hoverButton = FindHoverButton();
    if (hoverButton)
    {
...
void wxAuiNotebook::OnTabEndDrag(wxAuiTabCtrl* src_tabs, int src_idx)
{
    ...
    e.SetSelection(m_tabs.GetIdxFromWindow(src_page));
    e.SetOldSelection(e.GetSelection());
    e.SetEventObject(this);
    ProcessWindowEvent(e);
}
