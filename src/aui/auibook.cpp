...
{
    if (!TabHitTest(evt.m_x, evt.m_y) && !ButtonHitTest(evt.m_x, evt.m_y))
    {
...
wxAuiNotebook::LoadLayout(const wxString& name, wxAuiDeserializer& deserializer)
{
    ...
    for ( const auto& tab : tabs )
    {
        ...
        if ( tab.dock_direction == wxAUI_DOCK_CENTER )
        {
            // Special case: the pages vector is empty if we're using the
            // default order, in which case we can avoid doing anything if the
            // pages are already in the same order (which is a common case).
            if ( tab.pages.empty() )
            {
                ...
                if ( !mustRestore )
                {
                    ...
                    // already, so we don't have anything to do any more.
                    break;
                }
            }

            // Otherwise, remove all the existing pages and add them back in
            // the correct order below.
            tabMain->RemoveAll();

            // Make the vector with this correct order.
            for ( int i = 0; i < pageCount; ++i )
