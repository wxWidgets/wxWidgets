...
bool InitDarkMode()
{
    // In principle, dark mode is supported as far back as v1809 (build 17763).
    // But it is not fully tested for versions before Windows 10 v2004 ("20H1", build number 19041). Use caution!
...
void EnableForTLW(HWND hwnd)
{
    ...
    // DWMWA_USE_IMMERSIVE_DARK_MODE is 19 for v1809, but is 20 for later versions.
    // To set title bar black for both v1809 and later versions,
    // we try to call GetDwmSetWindowAttribute() with 20 first, and if it fails try again with 19.
