...
void wxDbgHelpDLL::LogError(const wxChar *func)
{
    OutputDebugString(wxString::Format(
        wxT("wxWidgets debughlp: %s() failed: %s\r\n"),
...
typedef PSTR ModName_t;
typedef wxWCharBuffer WCharBuf_t;
#else
typedef PCSTR ModName_t;
typedef const wxWCharBuffer WCharBuf_t;
#endif

#ifdef UNICODE

static BOOL CALLBACK wxMswEnmLddMdlsCallback1(
    ModName_t ModuleName, DWORD64 ModuleBase, ULONG ModuleSize, PVOID UserContext)
{
    wxMswEnmLddMdlsHelperStruct& alternate =
        *(wxMswEnmLddMdlsHelperStruct*)(UserContext);

    WCharBuf_t buf = wxConvLocal.cMB2WC(ModuleName, wxNO_LEN, NULL);
...
static BOOL CALLBACK wxMswEnmLddMdlsCallback2(
    ModName_t ModuleName, DWORD_PTR ModuleBase, ULONG ModuleSize, PVOID UserContext)
{
    wxMswEnmLddMdlsHelperStruct& alternate =
        *(wxMswEnmLddMdlsHelperStruct*)(UserContext);
...
BOOL wxDbgHelpDLL::SymFromAddrT(HANDLE hProcess, DWORD64 Address, PDWORD64 Displacement, wxPSYMBOL_INFO Symbol)
{
    ...
    if (SymFromAddr)
    {
        const size_t size = sizeof(SYMBOL_INFO) + Symbol->MaxNameLen*sizeof(CHAR);
        BYTE* symbolBuffer = new BYTE[size];
        ZeroMemory(symbolBuffer, size);
        PSYMBOL_INFO data = (SYMBOL_INFO*)(symbolBuffer);
        CopyMemory(data, Symbol, sizeof(SYMBOL_INFO)-sizeof(CHAR));
        ...
        const wxWCharBuffer buf =
            wxConvLocal.cMB2WC(data->Name, data->NameLen, NULL);

        // I know too brute but some header names SYMBOL_INFO::Index
