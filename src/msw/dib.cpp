...
{
    // note that sometimes GetObject() may return sizeof(DIBSECTION) for a
    // bitmap which is *not* a DIB section and the way to check for it is
...
HGLOBAL wxDIB::ConvertFromBitmap(HBITMAP hbmp)
{
    ...
    if ( !hDIB )
    {
        // this is an error which does risk to happen and which the user
        // may understand so let him know about it
