...
wxImage wxBitmap::ConvertToImage() const
{
    ...
    if (GetMask())
    {
        ...
        unsigned char* dataImage = image.GetData();
        ...
        for ( int y = 0; y < h; ++y )
        {
            ...
            for ( int x = 0; x < w; ++x )
            {
                ...
                if ( p.Pixel() )
                {
                    ...
                    if ( (dataImage[0] == MASK_RED) &&
                            (dataImage[1] == MASK_GREEN) &&
                                (dataImage[2] == MASK_BLUE) )
                    {
                        // we have to fudge the colour a bit to prevent
                        // this pixel from appearing transparent
                        dataImage[2] = MASK_BLUE_REPLACEMENT;
                    }
