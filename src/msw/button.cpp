...
void wxButton::MSWDrawXPBackground(WXDRAWITEMSTRUCT *wxdis)
{
    LPDRAWITEMSTRUCT lpDIS = (LPDRAWITEMSTRUCT)wxdis;
    HDC hdc = lpDIS->hDC;
    UINT state = lpDIS->itemState;
    ...
    else if ( HasCapture() || IsMouseInWindow() )
    {
        iState = PBS_HOT;
    }
    ...
    if ( wxUxThemeEngine::Get()->IsThemeBackgroundPartiallyTransparent(theme,
                                                                       ...
                                                                       iState) )
    {
        wxUxThemeEngine::Get()->DrawThemeParentBackground(GetHwnd(), hdc, &rectBtn);
    }
