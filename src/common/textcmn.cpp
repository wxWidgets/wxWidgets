...
// Mask methods
// ----------------------------------------------------------------------------
void wxTextCtrlBase::SetAssociatedMask(const wxMaskedEdit& mask)
{
        wxTextEntry::SetMask(mask);
        ChangeValue(m_maskCtrl.GetDefaultValue());
        SetBackgroundColour(m_maskCtrl.GetEmptyBackgroundColour());
        Connect(wxID_ANY, wxEVT_CHAR, wxKeyEventHandler(wxTextCtrlBase::OnKeyMask));
}

void wxTextCtrlBase::OnSetBackgroundMask()
{
    wxString string = GetValue();
    wxString formatString = m_maskCtrl.ApplyFormatCodes(string);
    wxString empty = m_maskCtrl.GetEmptyMask();

    if(empty.Cmp(string) == 0)
        SetBackgroundColour(m_maskCtrl.GetEmptyBackgroundColour());
    else if(m_maskCtrl.IsValid(formatString))
