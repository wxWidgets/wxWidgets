 ...
 * Name:    src/common/gsockbase.cpp
 * Purpose: GSocket: non-GUI-event-based part
...
#define GSOCKBASE_INPUT 1
#define GSOCKBASE_OUTPUT 2
...
int GSocketGUIFunctionsTableConcrete::TranslateEventCondition(GSocket* socket, GSocketEvent event) {
  ...
  {
    case GSOCK_LOST:       
    case GSOCK_INPUT:      return GSOCKBASE_INPUT; 
    case GSOCK_OUTPUT:   return GSOCKBASE_OUTPUT;
    case GSOCK_CONNECTION: return ((socket->m_server) ? GSOCKBASE_INPUT : GSOCKBASE_OUTPUT); 
...
void GSocketGUIFunctionsTableConcrete::SetNewCallback(GSocket* socket)
{
  ...
  CheckCurrentState(socket, GSOCKBASE_INPUT);
  CheckCurrentState(socket, GSOCKBASE_OUTPUT);
}
