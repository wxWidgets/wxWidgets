                 ...
                 , const bool useParensForNegatives)
{
    ...
    //test if the mask ended with a backslash and if it is the case m_mask is empty
    ...
    {
        // change the brace i.e #{3} ###
        m_mask = ChangeBraces(mask);
    }
...
wxString wxMaskedField::ChangeBraces(const wxString& mask)
{
    unsigned int it;
    ...
    for(it = 0; it < mask.Len(); it++)
    {
        ...
        {
            // if there is no previous opening or closing brace and
        ...
        {
            // if there is no closing brace before
            if(mask[it - 1] != '\\'
...
bool wxMaskedField::SetMask(const wxString& mask)
{
    ...
    if(mask.Cmp(wxEmptyString) != 0
    && ChangeBraces(mask).Cmp(wxEmptyString) == 0)
        res = false;
