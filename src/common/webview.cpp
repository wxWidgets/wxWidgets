...
void wxWebView::OnSyncScriptResult(wxCommandEvent& evt)
{
    wxSyncScriptExecution* exec = (wxSyncScriptExecution*) evt.GetClientData();
    exec->output = evt.GetString();
    exec->result = evt.GetInt();
}
