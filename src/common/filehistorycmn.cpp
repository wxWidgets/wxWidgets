...
wxString GetMRUEntryLabel(int n, const wxString& path, wxFileHistoryMenuPathStyle style, const wxString firstPath)
{
...
void wxFileHistoryBase::DoRefreshLabels()
{
    ...
    for ( size_t i = 0; i < numFiles; i++ )
    {
              ...
              node = node->GetNext() )
        {
            ...
            menu->SetLabel(m_idBase + i, GetMRUEntryLabel(i, m_fileHistory[i], m_menuPathStyle, firstPath));
        }
    }
}
...
void wxFileHistoryBase::RemoveFileFromHistory(size_t i)
{
          ...
          node = node->GetNext() )
    {
        ...
        {
            menu->SetLabel(m_idBase + j, GetMRUEntryLabel(j, m_fileHistory[j], m_menuPathStyle, firstPath));
        }
...
void wxFileHistoryBase::AddFilesToMenu(wxMenu* menu)
{
    ...
    {
        menu->Append(m_idBase + i, GetMRUEntryLabel(i, m_fileHistory[i], m_menuPathStyle, firstPath));
    }
}
