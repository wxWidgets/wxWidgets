...
void wxCursor::InitFromImage( const wxImage & image )
{
    ...
    else // no colour cursor support
    {
        ...
        else
        {
            ...
            // maskRaw = gdk_pixbuf_new_from_data(
...
static void UpdateCursors(const wxWindowList& list, GdkDisplay*& display)
{
    ...
    {
        wxWindow* win = *i;
        if (display == NULL && win->m_widget && gtk_widget_get_window(win->m_widget))
            display = gdk_window_get_display(gtk_widget_get_window(win->m_widget));
