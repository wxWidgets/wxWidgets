...
// Id:          $Id: toplevel.cpp 66633 2011-01-07 18:15:21Z PC $
...
static void wxgtk_window_set_urgency_hint (GtkWindow *win,
                                           gboolean setting)
{
    wxASSERT_MSG( gtk_widget_get_realized(GTK_WIDGET(win)), wxT("wxgtk_window_set_urgency_hint: GdkWindow not realized") );
...
size_allocate(GtkWidget*, GtkAllocation* alloc, wxTopLevelWindowGTK* win)
{
        ...
        win->m_oldClientHeight != alloc->height)
    {
        ...
        wxSize size(gtk_widget_get_allocated_width(win->m_widget),
                    gtk_widget_get_allocated_height(win->m_widget));
