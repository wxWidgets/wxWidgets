                                          ...
                                          const wxSize& origBest)
{
    ...
    if ( win->GetAutoLayout() )
    {
        ...
        if ( ppuX > 0 )
            best.x = minSize.x + wxSystemSettings::GetMetric(wxSYS_VSCROLL_X, const_cast<wxWindow*>(win));

        if ( ppuY > 0 )
            best.y = minSize.y + wxSystemSettings::GetMetric(wxSYS_HSCROLL_Y, const_cast<wxWindow*>(win));
    }
