...
$(OBJS)\erase.exe :  $(ERASE_OBJECTS)
	@%create $(OBJS)\erase.lbc
	@%append $(OBJS)\erase.lbc option quiet
	@%append $(OBJS)\erase.lbc name $^@
	@%append $(OBJS)\erase.lbc option caseexact
	@%append $(OBJS)\erase.lbc $(LDFLAGS) $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm
	@for %i in ($(ERASE_OBJECTS)) do @%append $(OBJS)\erase.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\erase.lbc library %i
...
$(OBJS)\erase_erase.obj :  .AUTODEPEND .\erase.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(ERASE_CXXFLAGS) $<
