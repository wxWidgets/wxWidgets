...
$(OBJS)\layout.exe :  $(LAYOUT_OBJECTS)
	...
	@%append $(OBJS)\layout.lbc $(LDFLAGS) $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm
	@for %i in ($(LAYOUT_OBJECTS)) do @%append $(OBJS)\layout.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\layout.lbc library %i
	@%append $(OBJS)\layout.lbc
	@for %i in () do @%append $(OBJS)\layout.lbc option stack=%i
	wlink @$(OBJS)\layout.lbc

$(OBJS)\layout_layout.obj :  .AUTODEPEND .\layout.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(LAYOUT_CXXFLAGS) $<
