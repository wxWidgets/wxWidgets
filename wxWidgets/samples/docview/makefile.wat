...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\docview.exe

$(OBJS)\docview.exe :  $(DOCVIEW_OBJECTS)
	...
	@%append $(OBJS)\docview.lbc $(LDFLAGS) $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm
	@for %i in ($(DOCVIEW_OBJECTS)) do @%append $(OBJS)\docview.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\docview.lbc library %i
	@%append $(OBJS)\docview.lbc
	@for %i in () do @%append $(OBJS)\docview.lbc option stack=%i
	wlink @$(OBJS)\docview.lbc

$(OBJS)\docview_docview.obj :  .AUTODEPEND .\docview.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(DOCVIEW_CXXFLAGS) $<
