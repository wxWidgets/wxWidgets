...
$(OBJS)\menu.exe :  $(MENU_OBJECTS)
	@%create $(OBJS)\menu.lbc
	@%append $(OBJS)\menu.lbc option quiet
	@%append $(OBJS)\menu.lbc name $^@
	@%append $(OBJS)\menu.lbc option caseexact
	@%append $(OBJS)\menu.lbc $(LDFLAGS) $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm
	@for %i in ($(MENU_OBJECTS)) do @%append $(OBJS)\menu.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\menu.lbc library %i
	@%append $(OBJS)\menu.lbc
	@for %i in () do @%append $(OBJS)\menu.lbc option stack=%i
	wlink @$(OBJS)\menu.lbc

$(OBJS)\menu_menu.obj :  .AUTODEPEND .\menu.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(MENU_CXXFLAGS) $<
