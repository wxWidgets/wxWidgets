...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\controls.exe

$(OBJS)\controls.exe :  $(CONTROLS_OBJECTS)
	...
	@%append $(OBJS)\controls.lbc $(LDFLAGS) $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm
	@for %i in ($(CONTROLS_OBJECTS)) do @%append $(OBJS)\controls.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\controls.lbc library %i
	@%append $(OBJS)\controls.lbc
...
$(OBJS)\controls_controls.obj :  .AUTODEPEND .\controls.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(CONTROLS_CXXFLAGS) $<
