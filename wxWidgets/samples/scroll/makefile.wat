...
$(OBJS)\scroll.exe :  $(SCROLL_OBJECTS)
	...
	@%append $(OBJS)\scroll.lbc $(LDFLAGS) $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm
	@for %i in ($(SCROLL_OBJECTS)) do @%append $(OBJS)\scroll.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\scroll.lbc library %i
	@%append $(OBJS)\scroll.lbc
	@for %i in () do @%append $(OBJS)\scroll.lbc option stack=%i
	wlink @$(OBJS)\scroll.lbc

$(OBJS)\scroll_scroll.obj :  .AUTODEPEND .\scroll.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(SCROLL_CXXFLAGS) $<
