...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\display.exe

$(OBJS)\display.exe :  $(DISPLAY_OBJECTS)
	...
	@%append $(OBJS)\display.lbc $(LDFLAGS) $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm
	@for %i in ($(DISPLAY_OBJECTS)) do @%append $(OBJS)\display.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\display.lbc library %i
	@%append $(OBJS)\display.lbc
	@for %i in () do @%append $(OBJS)\display.lbc option stack=%i
	wlink @$(OBJS)\display.lbc

$(OBJS)\display_display.obj :  .AUTODEPEND .\display.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(DISPLAY_CXXFLAGS) $<
