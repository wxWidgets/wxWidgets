...
/*
   unless special __WXSTANDLONE__ symbol is defined, assume that we're
   compiling wxWindows or an application using it -- strange as it may seem
   some parts of wxWindows may be used completely outside of the library and
   this symbol is useful in them to distinguish between the normal use and the
   "standalone" mode
 */
#ifndef __WXSTANDALONE__
#    define __WXWINDOWS__
#endif
...
/*
   In standalone mode we might define just a few things that we need in
   wx/setup.h and not everything as usual so skip the consistency check which
   only works for the "normal" use case
 */
#ifdef __WXWINDOWS__
