...
clean : .SYMBOLIC 
	...
	-del $(OBJS)\fractal.exe

$(OBJS)\fractal.exe :  $(FRACTAL_OBJECTS)
	...
	@%append $(OBJS)\fractal.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(FRACTAL_OBJECTS)) do @%append $(OBJS)\fractal.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\fractal.lbc library %i
	@%append $(OBJS)\fractal.lbc
	@for %i in () do @%append $(OBJS)\fractal.lbc option stack=%i
	wlink @$(OBJS)\fractal.lbc

$(OBJS)\fractal_fractal.obj :  .AUTODEPEND .\fractal.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(FRACTAL_CXXFLAGS) $<
