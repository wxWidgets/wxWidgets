...
ostream& operator<<(ostream& out, const wxFileName& fn)
{
...
void FileSystemWatcherTestCase::TestEventCreate()
{
    ...
    wxLogDebug("TestEventCreate tester created()");
...
public:
    ...
    bool CreateFile()
    {
        wxFile file(m_file.GetLongPath(), wxFile::write);
        return file.IsOpened() && m_file.FileExists();
    }
    ...
    bool RenameFile()
    {
        ...
        bool ret = wxRenameFile(m_file.GetLongPath(), m_new.GetLongPath());
        if (ret)
        {
            m_old = m_file;
            m_file = m_new;
    ...
    bool DeleteFile()
    {
        ...
        bool ret =  wxRemoveFile(m_file.GetLongPath());
        if (ret)
        {
            m_old = m_file;
            m_file = m_new;
    ...
    bool ModifyFile()
    {
        ...
        wxFile file(m_file.GetLongPath(), wxFile::write_append);
        CPPUNIT_ASSERT(file.IsOpened());
    ...
    static wxFileName RandomName(const wxFileName& base, int length = 10)
    {
        ...
        return wxFileName(base.GetLongPath(), s);
    }
...
{
public:
    EventHandler(wxEventLoopBase& loop) :
        eg(EventGenerator::Get()), m_loop(loop), m_count(0)
    {
        Bind(wxEVT_IDLE, &EventHandler::OnIdle, this);
        Bind(wxEVT_FSWATCHER, &EventHandler::OnFileSystemEvent, this);
    }
...
{
    m_loop = new wxEventLoop();
    m_eventHandler = new EventHandler(*m_loop);
}
...
void FileSystemWatcherTestCase::TestEvents()
{
    wxLogDebug("TestEvents()");

    class EventTester : public EventHandler
    {
    public:
        EventTester(wxEventLoopBase& loop) :
            EventHandler(loop)
        {
        }
        ...
        virtual bool Action()
        {
            ...
            case 3:
            {
                ...
                CPPUNIT_FAIL("No events from watcher during 3 seconds!");
                break;
            }
            } // switch (m_count)
        ...
        virtual void OnFileSystemEvent(wxFileSystemWatcherEvent& evt)
        {
            EventHandler::OnFileSystemEvent(evt);
            ...
            for ( ; it != m_events.end(); ++it)
            {
                if ((*it)->GetChangeType() == wxFSW_EVENT_CREATE)
                {
                    e = *it;
                    break;
                }
            ...
            // ok, lets check that event is correct
            CPPUNIT_ASSERT_EQUAL(eg.m_file, e->GetPath());
