...
- Remaining jobs on that SHA expose a second, distinct GTK editor-lifetime crash (99278538111 and 99278538233): after `OnFocus()` deletes the secondary, GTK redraws before the outer selection guard runs. The correction retires editor slots at `RemoveChild()`, the existing ownership boundary, and tests their state immediately after deletion. It adds no data member or virtual slot; the earlier commit-focus guard remains intact. Final cross-platform validation is pending at this entry.
- Mac Intel/Universal expose two renderer-reentrancy fixtures which assumed a native repaint during selection and an RTL fixture which contradicted the documented Mac no-op contract. The renderer fixtures now invoke the real `DrawItems()`/cell renderer from a test-local virtual `Update()` boundary, preserving all removal/selection assertions. They do not claim native paint delivery. Mac RTL checks the documented default direction and both hit-test mappings; other ports retain their actual RTL/LTR checks.
- Fedora CircleCI 6670/GCC 16 diagnoses a speculative inlining path from a plain `wxPanel` into the unrelated `SizeCallbackPanel` fixture (`-Werror=array-bounds`). Initializing the three candidate sizes during construction avoids that path; their previous unconstrained minimum size is explicitly restored and checked. MSW Debug builds and all four affected cases pass 32 assertions; GTK3/GCC 11.4 compiles/links. No assertion or warning is disabled. GCC 16 remote confirmation remains required.
- DirCtrl's exact CI-shaped short temporary directory is now reproduced locally. Fixing long-name lookup also exposes stale aliases after rename and a canonical directory longer than `MAX_PATH`. The scoped repair covers longest-existing-prefix lookup plus private extended Windows paths at this control's four active enumeration boundaries and related existence queries. Public/default paths stay unchanged; no global file API is modified. This expands the same confirmed filesystem regression, not a claim that every long-path operation (such as rename beyond `MAX_PATH`) is qualified. Final short-TMP execution remains pending at this entry.

### Final local checks of the follow-up batch

- The frozen source set through 53394a7755 builds `test_gui` and `minimal` in both WinUI Release linkages with `/warnaserror`. MSW Debug `test_gui` also builds with `/warnaserror`; GTK3/GCC 11.4 compiles and links the full target (33 actions), without launching a GUI. Logs: `audit101-final-ci-build.log` in each build tree. The source fixes are separate commits: book fixture 98af229145, DirCtrl ea7407d154, and editor ownership/portable fixtures 53394a7755.
- Full PropertyGrid passes on the final binaries: WinUI shared **1225 assertions / 2 cases**, static **1230/2**, MSW **1168/2**, Qt 5.15.2 **902/2**. Logs: `audit101-final-ci-propgrid.log` in WinUI/MSW and `audit101-qt-propgrid-final.log` in Qt. The four isolated static sections also pass: editor deletion 11 assertions, renderer selection add 7, removal 8, and RTL mapping 6. These supersede earlier intermediate-binary counts, which can differ with legitimate paint callbacks.
- Full filesystem tests pass **254 assertions / 7 cases** in MSW and both WinUI linkages. Qt passes the same **254/7** with TEMP/TMP pointing to a real short-name alias, including the 39-assertion alias/rebuild/rename case and the 32-assertion mutation case. The selected Unicode file has a canonical parent of 269 characters. No capability WARN/early return or timeout is counted as this success. Qt log: `audit101-qt-filesystem-short-temp-final.log`; other logs: `audit101-final-ci-filesystemctrl.log` (shared WinUI uses `filesystem.log`).
- All four affected book fixtures pass **32 assertions / 4 cases** on MSW Debug (`audit101-final-ci-book.log`). All GUI checks above use private desktops and `WX_UI_TESTS=0`, with no global input injection. Cocoa/GTK runtime, GCC 14/16 and Qt 6 remain remote requirements on the newly published batch; plan 101 is not yet DONE.
- Final WinUI runtime smoke and Supported V0 CTests both pass in shared and static (2/2 gates; Supported V0 1660 assertions / 92 cases). Logs: shared `audit101-final-ci-gates.log`, static `audit101-final-ci-smoke-supported.log` and its JUnit report. This is local evidence for the follow-up batch, not the result of its future remote run.

### Remote confirmation on 8fd4d2426b

- macOS ARM ASAN run 33322287399, job 99286252614: build and both runtime steps succeed, including **355469 assertions / 843 GUI cases** and **1231328/863 non-GUI**. The executable is linked with AddressSanitizer. The job retains its existing excluded-test selection and `detect_container_overflow=0`; this is not an unfiltered or physical-input qualification.
- MSW 32-bit run 33322287369, job 99286252422: GCC 14 compilation and both test steps succeed under Wine 10, with **354224/1009 GUI** and **1150137/831 non-GUI**. Existing Wine exclusions remain explicit. The previously failing compiler fixture is no longer rejected.
- Fedora CircleCI build 6671, job 2afaf10e-53c5-4c0a-b3f3-d8aa685b68cc: GCC 16.2 builds the tests with warnings treated as errors; the non-GUI run passes **1206609 assertions / 872 cases**. This job does not execute GUI tests, so it confirms compilation of the book fixture, not Fedora GUI behaviour.
- Compact job metadata and runtime evidence are retained in `F:\wxwinui-pr26890-audit101-ci-8fd4d2426b`. The subsequent snapshot also reports GTK gcc7/static, GTK clang, GTK ASAN and macOS dynamic Release jobs successful. Qt 6.10 job 99286252827 fails and is being diagnosed; 101 remains OPEN instead of treating these partial successes as global CI completion.
- The Qt 6.10 job now reaches three later failures: two auxiliary-window tests call the unimplemented Qt `ShowWithoutActivating()`, and a Toolbook property-sheet page crashes without an image. The latter reproduces on local Qt 5.15.2, with 36 assertions before the crash; the native debugger stack proves `wxToolBarTool::SetIcon()` dereferences a null pixmap while adding the image-less page. The scoped correction maps an empty bitmap to an empty `QIcon`, with a new native-widget check for initial emptiness, setting an icon and clearing it again. Original property-sheet assertions remain unchanged. Logs: `audit101-qt-propertysheet-before.log` and `audit101-qt-propertysheet-stack-firstchance.log`. Validation of these newly reachable failures is pending.
- The Qt fixes now build with `/warnaserror` on Qt 5.15.2/MSVC. `ShowWithoutActivating()` temporarily preserves/restores the native Qt attribute while retaining the virtual wx show path, and guards owner/widget lifetime after callbacks. The two unchanged auxiliary cases and the new nonactivation/normal-activation regression pass **40 assertions / 3 cases**, with event pumping before the nonactivation checks. This is Qt active-window evidence on a private Windows desktop, not qualification of all window managers. Log: `qt5-auxiliary-and-nonactivating-after.log` in the remote-evidence directory above.
- All Toolbook cases pass **71 assertions / 11 cases**, including the new 17-assertion icon add/clear regression. The original property-sheet construction section, previously crashing, now passes **71 assertions** across its advertised book styles. Logs: `audit101-qt-toolbook-after.log` and `audit101-qt-propertysheet-construction-after.log` in the Qt build tree. No assertion or case is disabled.
- Running the complete property-sheet case after removing the crash exposes **25 further failed assertions** (combined property-sheet/Toolbook run: 658/683 assertions pass). They concern Qt notebook veto/removal, min/max-size restoration and callback/focus fixtures; they remain OPEN within 101. Log: `audit101-qt-propertysheet-toolbook-after.log`. The passing construction section does not qualify the complete property-sheet case.
