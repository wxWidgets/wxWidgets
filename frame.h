...
public:
    ...
    */
    ...
    /**
        ...
        Example: creating a top-level frame with typical defaults:

        @code
        wxFrame* frame = new wxFrame(
            NULL,                  // parent: top-level window (no parent)
            wxID_ANY,              // id: let wxWidgets assign a unique ID
            "My Application",      // title: caption shown in the title bar
            wxDefaultPosition,     // pos: let the OS choose the position
            wxSize(800, 600),      // size: initial width x height in pixels
            wxDEFAULT_FRAME_STYLE, // style: standard decorations (borders, buttons)
            "MyFrame"              // name: internal identifier (for debugging)
        );
        @endcode
